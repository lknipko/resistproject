...
export async function generateStaticParams() {
  return getStaticParams('environment')
}
...
export async function generateMetadata({
  ...
  const page = getPageBySlug('environment', slug)
...
export default async function EnvironmentTopicPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  ...
  const resolvedData = await getCachedResolvedContent('environment', slug)
  ...
  return (
    <article>
      ...
      {resolvedData.version > 0 && (
        <div className="mt-6 p-4 bg-forest-50 border border-forest-200 rounded-lg text-sm text-forest-800">
